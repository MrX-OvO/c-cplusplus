...
template<typename T>
SingletonLazy<T>* SingletonLazy<T>::m_instance = nullptr;

// 懒汉模式线程安全问题
// 使用 mutex ，双重检查锁定
template<typename T>
class SingletonLazyMutex{
  public:
...
template<typename T>
std::mutex SingletonLazyMutex<T>::m_mutex;

// 使用原子变量 atomic
template<typename T>
class SingletonLazyAtomic{
  public:
...
template<typename T>
std::atomic<SingletonLazyAtomic<T>*> m_instance;

template<typename T>
std::mutex SingletonLazyAtomic<T>::m_mutex;
